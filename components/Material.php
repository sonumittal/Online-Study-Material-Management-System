...
<div class="align-items-center p-3 my-3 rounded shadow-sm well">
    <h5>Select Course for study material:</h5>
...
$query="select * from study_material where subject_type='$SubjectType' AND subject_course_name='$SubjectCourseName' AND subject_name='$SubjectName' order by id desc";
$row=mysqli_query($connection,$query);
$no=1;
while($result=mysqli_fetch_array($row))
{
...
echo "<td>".$result['7']."</td>";
$file=$result['6'];
echo "<td><a href='admin_panel/uploaded_files/material/".$file."' target='blank'>".$file."</a></td></tr>";
$no=$no+1;
}}
?>
 </tbody>
